  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">{days.map((d, i) => <div key={d} className="flex items-center gap-2 text-sm sm:gap-3"><Switch defaultChecked={i < 6} /><span className="w-9 shrink-0 text-foreground-secondary">{d}</span><Input type="time" defaultValue={i === 5 ? "10:00" : "09:00"} className="h-8 min-w-0 flex-1 sm:w-28 sm:flex-none" /><span className="text-muted">–</span><Input type="time" defaultValue={i === 5 ? "16:00" : "19:00"} className="h-8 min-w-0 flex-1 sm:w-28 sm:flex-none" /></div>)}</div>
...
export function NotificationsSection() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-[minmax(0,1fr)_repeat(4,44px)] items-center gap-1 border-b border-border pb-2 text-[10px] font-semibold uppercase tracking-wider text-muted sm:grid-cols-[minmax(0,1fr)_repeat(4,64px)] sm:gap-2 sm:text-[11px]"><span>Event</span><span className="text-center">In-app</span><span className="text-center">Email</span><span className="text-center">Push</span><span className="text-center">Slack</span></div>
        {items.map((it, i) => <div key={it} className="grid grid-cols-[minmax(0,1fr)_repeat(4,44px)] items-center gap-1 border-b border-border py-2.5 text-sm sm:grid-cols-[minmax(0,1fr)_repeat(4,64px)] sm:gap-2"><span>{it}</span><span className="flex justify-center"><Switch defaultChecked /></span><span className="flex justify-center"><Switch defaultChecked={i < 3 || i > 5} /></span><span className="flex justify-center"><Switch defaultChecked={i === 0 || i === 2} /></span><span className="flex justify-center"><Switch defaultChecked={i < 2} /></span></div>)}
